...
class GroveLcd {
  ...
  void initialise() {
    sleep(INIT_SLEEP);
    _writeReg(GroveLcdDefinitions.LCD_CMD,
        <int>[0xFD]); // Unlock OLED driver IC MCU
    // interface from entering command.
    // i.e: Accept commands
    sleep(INIT_SLEEP);
    _writeReg(GroveLcdDefinitions.LCD_CMD, <int>[0x12]);
    sleep(INIT_SLEEP);
    error = _writeReg(
        GroveLcdDefinitions.LCD_CMD, <int>[0xAE]); // Set display off
    sleep(INIT_SLEEP);
    _writeReg(
        GroveLcdDefinitions.LCD_CMD, <int>[0xA8]); // set multiplex ratio
    sleep(INIT_SLEEP);
    error = _writeReg(GroveLcdDefinitions.LCD_CMD, <int>[0x5F]); // 96
    sleep(INIT_SLEEP);
    error = _writeReg(
        GroveLcdDefinitions.LCD_CMD, <int>[0xA1]); // set display start line
    sleep(INIT_SLEEP);
    error = _writeReg(GroveLcdDefinitions.LCD_CMD, <int>[0x00]); //
    sleep(INIT_SLEEP);
    error = _writeReg(
        GroveLcdDefinitions.LCD_CMD, <int>[0xA2]); // set display offset
    sleep(INIT_SLEEP);
    error = _writeReg(GroveLcdDefinitions.LCD_CMD, <int>[0x60]);
    sleep(INIT_SLEEP);
    error = _writeReg(GroveLcdDefinitions.LCD_CMD, <int>[0xA0]); // set remap
    sleep(INIT_SLEEP);
    error = _writeReg(GroveLcdDefinitions.LCD_CMD, <int>[0x46]);
    sleep(INIT_SLEEP);
    error = _writeReg(
        GroveLcdDefinitions.LCD_CMD, <int>[0xAB]); // set vdd internal
    sleep(INIT_SLEEP);
    error = _writeReg(GroveLcdDefinitions.LCD_CMD, <int>[0x01]); //
    sleep(INIT_SLEEP);
    error =
        _writeReg(GroveLcdDefinitions.LCD_CMD, <int>[0x81]); // set contrasr
    sleep(INIT_SLEEP);
    error = _writeReg(GroveLcdDefinitions.LCD_CMD, <int>[0x53]); // 100 nit
    sleep(INIT_SLEEP);
    error = _writeReg(
        GroveLcdDefinitions.LCD_CMD, <int>[0xB1]); // Set Phase Length
    sleep(INIT_SLEEP);
    error = _writeReg(GroveLcdDefinitions.LCD_CMD, <int>[0X51]); //
    sleep(INIT_SLEEP);
    error = _writeReg(GroveLcdDefinitions.LCD_CMD,
        <int>[0xB3]); // Set Display Clock Divide Ratio/Oscillator
    // Frequency
    sleep(INIT_SLEEP);
    error = _writeReg(GroveLcdDefinitions.LCD_CMD, <int>[0x01]); //
    sleep(INIT_SLEEP);
    error = _writeReg(GroveLcdDefinitions.LCD_CMD, <int>[0xB9]); //
    sleep(INIT_SLEEP);
    error =
        _writeReg(GroveLcdDefinitions.LCD_CMD, <int>[0xBC]); // set pre_charge
    // voltage/VCOMH
    sleep(INIT_SLEEP);
    error = _writeReg(GroveLcdDefinitions.LCD_CMD, <int>[0x08]); // (0x08);
    sleep(INIT_SLEEP);
    error = _writeReg(GroveLcdDefinitions.LCD_CMD, <int>[0xBE]); // set VCOMH
    sleep(INIT_SLEEP);
    error = _writeReg(GroveLcdDefinitions.LCD_CMD, <int>[0X07]); // (0x07);
    sleep(INIT_SLEEP);
    error = _writeReg(
        GroveLcdDefinitions.LCD_CMD, <int>[0xB6]); // Set second pre-charge
    // period
    sleep(INIT_SLEEP);
    error = _writeReg(GroveLcdDefinitions.LCD_CMD, <int>[0x01]); //
    sleep(INIT_SLEEP);
    ...
    sleep(INIT_SLEEP);
    error = _writeReg(GroveLcdDefinitions.LCD_CMD, <int>[0X62]); // (0x62);
    sleep(INIT_SLEEP);
    error = _writeReg(
        GroveLcdDefinitions.LCD_CMD, <int>[0xA4]); // Set Normal Display Mode
    sleep(INIT_SLEEP);
    error = _writeReg(
        GroveLcdDefinitions.LCD_CMD, <int>[0x2E]); // Deactivate Scroll
    sleep(INIT_SLEEP);
    error = _writeReg(
        GroveLcdDefinitions.LCD_CMD, <int>[0xAF]); // Switch on display
    sleep(INIT_SLEEP);

    // Row Address
    error = _writeReg(
        GroveLcdDefinitions.LCD_CMD, <int>[0x75]); // Set Row Address
    sleep(INIT_SLEEP);
    error = _writeReg(GroveLcdDefinitions.LCD_CMD, <int>[0x00]); // Start 0
    sleep(INIT_SLEEP);
    error = _writeReg(GroveLcdDefinitions.LCD_CMD, <int>[0x5f]); // End 95
    sleep(INIT_SLEEP);

    // Column Address
    error = _writeReg(
        GroveLcdDefinitions.LCD_CMD, <int>[0x15]); // Set Column Address
    sleep(INIT_SLEEP);
    error = _writeReg(
        GroveLcdDefinitions.LCD_CMD, <int>[0x08]); // Start from 8th Column of
    // driver IC. This is 0th
    // Column for OLED
    sleep(INIT_SLEEP);
    error = _writeReg(
        GroveLcdDefinitions.LCD_CMD, <int>[0x37]); // End at  (8 + 47)th
  ...
  MraaReturnCode draw(Uint8List data, int bytes) {
    ...
    for (int row = 0; row < bytes; row++) {
      for (int col = 0; col < 8; col += 2) {
        ...
        _writeReg(GroveLcdDefinitions.LCD_DATA, <int>[value]);
  ...
  MraaReturnCode setCursor(int row, int column) {
    ...
    _writeReg(
        GroveLcdDefinitions.LCD_CMD, <int>[0x15]); /* Set Column Address */
    sleep(CMD_SLEEP);
    _writeReg(GroveLcdDefinitions.LCD_CMD, <int>[0x08 + (column * 4)]);
    /* Start Column:
       Start from 8 */
    sleep(CMD_SLEEP);
    _writeReg(GroveLcdDefinitions.LCD_CMD, <int>[0x37]); /* End Column */
    sleep(CMD_SLEEP);
    // Row Address
    _writeReg(GroveLcdDefinitions.LCD_CMD, <int>[0x75]); /* Set Row Address */
    sleep(CMD_SLEEP);
    _writeReg(
        GroveLcdDefinitions.LCD_CMD, <int>[0x00 + (row * 8)]); /* Start Row*/
    sleep(CMD_SLEEP);
    _writeReg(
        GroveLcdDefinitions.LCD_CMD, <int>[0x07 + (row * 8)]); /* End Row*/
    sleep(CMD_SLEEP);
  ...
  void drawBitMap(List<int> bitmap, int bytes) {
    ...
    for (int i = 0; i < bytes; i++) {
      for (int j = 0; j < 8; j = j + 2) {
        ...
        _writeReg(GroveLcdDefinitions.LCD_DATA, <int>[c]);
      }
    }
    if (wasVertical) {
      _setVerticalMode();
  ...
  MraaReturnCode _writeChar(int value) {
    ...
    for (int row = 0; row < 8; row = row + 2) {
      for (int col = 0; col < 8; col++) {
        ...
        final int bitOne =
            ((GroveLcdDefinitions.basicFont[calcValue - 32][row]) >> col) &
                0x1;
        final int bitTwo = ((GroveLcdDefinitions.basicFont[calcValue - 32]
                    [row + 1]) >>
                col) &
            0x1;
        ...
        rv = _writeReg(GroveLcdDefinitions.LCD_DATA, <int>[data]);
        sleep(CMD_SLEEP);
      }
    }
    return rv;
  ...
  MraaReturnCode _setNormalDisplay() =>
      _writeReg(GroveLcdDefinitions.LCD_CMD, <int>[
        GroveLcdDefinitions.DISPLAY_CMD_SET_NORMAL
      ]); // set to normal display '1' is ON

  MraaReturnCode _setHorizontalMode() {
    MraaReturnCode rv = MraaReturnCode.success;
    rv = _writeReg(GroveLcdDefinitions.LCD_CMD, <int>[0xA0]); // remap to
    sleep(CMD_SLEEP);
    rv = _writeReg(
        GroveLcdDefinitions.LCD_CMD, <int>[0x42]); // horizontal mode
    sleep(CMD_SLEEP);
    ...
    rv = _writeReg(
        GroveLcdDefinitions.LCD_CMD, <int>[0x75]); // Set Row Address
    sleep(CMD_SLEEP);
    rv = _writeReg(GroveLcdDefinitions.LCD_CMD, <int>[0x00]); // Start 0
    sleep(CMD_SLEEP);
    rv = _writeReg(GroveLcdDefinitions.LCD_CMD, <int>[0x5f]); // End 95
    sleep(CMD_SLEEP);
    ...
    rv = _writeReg(
        GroveLcdDefinitions.LCD_CMD, <int>[0x15]); // Set Column Address
    sleep(CMD_SLEEP);
    rv = _writeReg(GroveLcdDefinitions.LCD_CMD,
  ...
  MraaReturnCode _setVerticalMode() {
    ...
    rv = _writeReg(GroveLcdDefinitions.LCD_CMD, <int>[0xA0]); // remap to
    sleep(CMD_SLEEP);
    rv = _writeReg(GroveLcdDefinitions.LCD_CMD, <int>[0x46]); // Vertical mode
    sleep(CMD_SLEEP);
